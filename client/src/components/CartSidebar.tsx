...
export default function CartSidebar({ isOpen, onClose }: CartSidebarProps) {
  const { cartItems, totalAmount, updateCart, removeFromCart, clearCart } = useCart();
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {cartItems.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">Add some beautiful handcrafted items to get started!</p>
            </div>
          ) : (
              ...
              {cartItems.map((item: any) => (
                <div key={item.id} className="flex items-center space-x-4 border-b pb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-600">{item.product.material}</p>
