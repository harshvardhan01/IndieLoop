        ...
        {orders.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              You haven't placed any orders yet. Start exploring our beautiful handcrafted products!
            </p>
            <Link href="/">
                      ...
                      <div className="space-y-2">
                        {order.items.map((item, index) => (
                          <div key={index} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-b-0">
                            <div>
                              <span className="text-sm text-gray-600">Product ID: {item.productId.slice(-8)}</span>
                    ...
                    {order.trackingNumber && (
                      <div className="bg-gray-50 rounded-lg p-4">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          Use this tracking number to monitor your shipment progress
                        </p>
                      </div>
                    )}
                      ...
                      <div className="flex items-center justify-between">
                        <div className={`flex flex-col items-center ${
                          ['pending', 'processing', 'shipped', 'delivered'].includes(order.status.toLowerCase()) 
                            ? 'text-craft-brown' : 'text-gray-400'
                        }`}>
                          <div className={`w-3 h-3 rounded-full ${
                            ['pending', 'processing', 'shipped', 'delivered'].includes(order.status.toLowerCase())
                              ? 'bg-craft-brown' : 'bg-gray-300'
                          }`} />
